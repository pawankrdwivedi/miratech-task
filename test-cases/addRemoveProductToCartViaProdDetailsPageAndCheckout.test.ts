...
test('e2e-Add \Remove Product to Cart from product details page and checkout', { tag: '@Smoke'}, async ({ loginPage, commonPage, homePage, yourCartPage, yourInformationPage,checkoutPage }) => {
    ...
    await test.step('3. Open product details, add to cart and then remove it', async () => {
        await homePage.openBackPackDetailsPage();
        await commonPage.verifyProductName (PRODCUT_NAME);
        await commonPage.clickAddToCartButton();
        await commonPage.clickRemoveButton();
