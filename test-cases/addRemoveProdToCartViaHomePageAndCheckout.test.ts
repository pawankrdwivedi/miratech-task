...
const PRODCUT_NAME="Sauce Labs Bike Light";

test('e2e-Add /Remove Product to Cart from home page and checkout', { tag: '@Smoke'}, async ({ loginPage, commonPage, homePage, yourCartPage, yourInformationPage,checkoutPage }) => {
    ...
    await test.step('Add a product to cart and then Remove it.', async () => {
        await homePage.addBackPackToCart();
        await homePage.removeBackPackFromCart();
    });

    await test.step('Add another product to cart and click on shopping link.', async () => {
